...
Novamente, como no exemplo da figura~\ref{fig:ex07:matrices/Server:62:89}, é importante chamar a operação \code{logout} quando a conexão não for mais utilizada para facilitar com que os recursos alocados para manutenção da autenticação sejam liberados (linha~\ref{lin:ex07:matrices/Server:logoutconn}).

\inputexrange{ex07}{matrices/Server}{62}{89}{Servidor de matrizes com login por certificado.}

\section{Autenticação Compartilhada} \label{sec:sharedauth}
...
A figura~\ref{fig:ex09:matrices/Server:55:83} ilustra a implementação da fábrica de matrizes fazendo uso da identificação da origem das chamadas para recusar chamadas de origens não autenticadas.
...
\inputexrange{ex09}{matrices/Server}{55}{83}{Fábrica de matrizes com identificação de chamada.}

Adotamos essa abordagem para adaptar as implementações das operações dos objetos de matrizes, como ilustrado na figura~\ref{fig:ex09:matrices/Server:20:53}.
Na linha~\ref{lin:ex09:matrices/Server:authimpl} declaramos uma operação privada que faz a validação da origem da chamada tal como na figura~\ref{fig:ex09:matrices/Server:55:83}.
...
\inputexrange{ex09}{matrices/Server}{20}{53}{Operações das matrizes com identificação de chamada.}
...
A figura~\ref{fig:ex12:matrices/Server:91:131} ilustra a adaptação do servidor de matrizes para obter uma refência para o serviço de provedor de dados (linha~\ref{lin:ex12:matrices/Server:getref}) e repassá-la para a fábrica de matrizes (linha~\ref{lin:ex12:matrices/Server:passref}).

\inputexrange{ex12}{matrices/Server}{91}{131}{Servidor de fábrica de matrizes obtendo referência para serviço de dados.}

A figura~\ref{fig:ex12:matrices/Server:55:89} ilustra as alterações na fábrica de matrizes para armazenar a referência do serviço (linha~\ref{lin:ex12:matrices/Server:saveref}) e utilizá-la para obter os dados (linha~\ref{lin:ex12:matrices/Server:callservice}) para criação de uma matriz (linha~\ref{lin:ex12:matrices/Server:creatematrix}).
...
\inputexrange{ex12}{matrices/Server}{55}{89}{Fábrica de matrizes utilizando serviço de dados.}
...
Isso é ilustrado na figura~\ref{fig:ex13:matrices/Server:70:94}.
...
A figura~\ref{fig:ex13:matrices/Server:107:153} ilustra como alteramos a operação de criação de matrizes para registrar as matrizes para o devido descarte em decorrência da notificação de logout.
...
Isso será usado para que a matriz possa se retirar do mapa de matrizes da fábrica quando ela for descartada como ilustrado na figura~\ref{fig:ex13:matrices/Server:155:175}.
...
\inputexrange{ex13}{matrices/Server}{155}{175}{Operação de descarte da matriz.}

Por fim, a figura~\ref{fig:ex13:matrices/Server:35:68} ilustra a nova implementação da classe \code{MatrixServant} em que o construtor recebe uma referência para o objeto de fábrica e o login que solicitou a criação da matriz(linha~\ref{lin:ex13:matrices/Server:savefactory} e \ref{lin:ex13:matrices/Server:savelogin}).
